...
window.MERCENARY_PHASE_STYLES = {
    "Champion's Initiation": { label: "Champion's Initiation", color: "#10b981", bg: "rgba(16,185,129,0.15)", border: "rgba(16,185,129,0.4)", gradient: "linear-gradient(135deg, #059669, #10b981)" },
    "Epic Initiation": { label: "Epic Initiation", color: "#a855f7", bg: "rgba(168,85,247,0.15)", border: "rgba(168,85,247,0.4)", gradient: "linear-gradient(135deg, #7e22ce, #a855f7)" },
    "Legend's Initiation": { label: "Legend's Initiation", color: "#06b6d4", bg: "rgba(6,182,212,0.15)", border: "rgba(6,182,212,0.4)", gradient: "linear-gradient(135deg, #0e7490, #06b6d4)" },
    "Fearless": { label: "Fearless", color: "#f43f5e", bg: "rgba(244,63,94,0.15)", border: "rgba(244,63,94,0.4)", gradient: "linear-gradient(135deg, #be123c, #f43f5e)" }
};
      ...
      <div class="card" style="max-width:1200px; margin:0 auto; animation: fadeIn 0.3s ease; position:relative;">
        <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:15px; border-bottom:1px solid var(--border); padding-bottom:10px;">
          <h2 style="color:var(--accent); margin:0; display:flex; align-items:center; flex-wrap:wrap; gap:10px;">
            🐻 Multi-BT Donations
            <button onclick="views.bearTrapAdmin()" style="background:linear-gradient(135deg, #10b981, #059669); color:#fff; border:none; padding:5px 12px; border-radius:6px; cursor:pointer; font-weight:bold; font-size:12px; margin-left:10px; box-shadow:0 2px 8px rgba(16,185,129,0.3);">📊 BT Tracker</button>
            <button onclick="document.getElementById('btLookupModal').style.display='block'" style="background:var(--card-bg); color:var(--text-main); border:1px solid var(--accent); padding:5px 12px; border-radius:6px; cursor:pointer; font-weight:bold; font-size:12px; margin-left:10px;">🔍 Lookup</button>
            <div style="position:relative; display:inline-block; margin-left:10px;">
              <button onclick="const m = this.nextElementSibling; if(m.style.display==='flex'){m.style.display='none';}else{document.querySelectorAll('.admin-dropdown-menu').forEach(d=>d.style.display='none'); m.style.display='flex';} event.stopPropagation();" style="background:var(--card-bg); color:var(--text-main); border:1px solid var(--border); padding:5px 12px; border-radius:6px; cursor:pointer; font-weight:bold; font-size:12px; display:flex; align-items:center; gap:5px; transition:0.2s;" onmouseover="this.style.borderColor='var(--accent)'" onmouseout="this.style.borderColor='var(--border)'">
                ⚙️ Options ▾
              </button>
              <div class="admin-dropdown-menu" style="display:none; flex-direction:column; gap:6px; position:absolute; top:100%; left:0; margin-top:6px; background:var(--card-bg); border:1px solid var(--border); border-radius:10px; padding:10px; min-width:200px; box-shadow:0 10px 30px rgba(0,0,0,0.6); z-index:1000;" onclick="event.stopPropagation();">
                <button onclick="document.getElementById('btCrownModal').style.display='block'; this.parentElement.style.display='none';" style="background:rgba(16,185,129,0.1); color:#10b981; border:1px solid rgba(16,185,129,0.3); padding:8px 12px; border-radius:6px; cursor:pointer; font-weight:bold; font-size:12px; text-align:left; transition:0.2s;" onmouseover="this.style.background='rgba(16,185,129,0.2)'" onmouseout="this.style.background='rgba(16,185,129,0.1)'">👑 Crown Winner</button>
                <button onclick="window.resetBearTrapWinners(); this.parentElement.style.display='none';" style="background:rgba(239,68,68,0.1); color:var(--danger); border:1px solid rgba(239,68,68,0.3); padding:8px 12px; border-radius:6px; cursor:pointer; font-weight:bold; font-size:12px; text-align:left; transition:0.2s;" onmouseover="this.style.background='rgba(239,68,68,0.2)'" onmouseout="this.style.background='rgba(239,68,68,0.1)'">🔄 Reset BT Winners</button>
                <div style="height:1px; background:var(--border); margin:2px 0;"></div>
                <button onclick="window.resetBearTrapEvent(); this.parentElement.style.display='none';" style="background:linear-gradient(135deg, #ef4444, #dc2626); color:#fff; border:none; padding:8px 12px; border-radius:6px; cursor:pointer; font-weight:bold; font-size:12px; text-align:left; transition:0.2s; box-shadow:0 2px 8px rgba(239,68,68,0.3);">🔄 Reset BT Event</button>
                <button onclick="document.getElementById('btResetPlayerModal').style.display='block'; document.getElementById('btResetPlayerModalOverlay').style.display='block'; this.parentElement.style.display='none';" style="background:rgba(239,68,68,0.1); color:var(--danger); border:1px solid rgba(239,68,68,0.3); padding:8px 12px; border-radius:6px; cursor:pointer; font-weight:bold; font-size:12px; text-align:left; transition:0.2s;" onmouseover="this.style.background='rgba(239,68,68,0.2)'" onmouseout="this.style.background='rgba(239,68,68,0.1)'">🗑️ Reset Player</button>
                <div style="height:1px; background:var(--border); margin:2px 0;"></div>
                <button onclick="window.openBtDbEditor(); this.parentElement.style.display='none';" style="background:rgba(139,92,246,0.1); color:#a78bfa; border:1px solid rgba(139,92,246,0.3); padding:8px 12px; border-radius:6px; cursor:pointer; font-weight:bold; font-size:12px; text-align:left; transition:0.2s;" onmouseover="this.style.background='rgba(139,92,246,0.2)'" onmouseout="this.style.background='rgba(139,92,246,0.1)'">🛠️ Open DB Editor</button>
              </div>
            </div>
          </h2>
        ...
        window._scheduleCountdownTimer = setInterval(() => {
          ...
          if (liveSched && Array.isArray(liveSched.events) && liveSched.events.length > 0) {
            for (let d = 0; d < 7; d++) {
              ...
              const matchingEvents = liveSched.events.filter(ev => {
                const evDate = window.parseScheduleEventDate(ev.dateStr);
                if (!evDate) return false;
                return evDate.toDateString() === dayObj.toDateString();
              });
              ...
              days.push({
                dateStr: isToday ? `Today • ${dayStr}` : dayStr,
                dateObj: dayObj,
                isToday: isToday,
...
// ============================================================================
// AUTOMATIC REAL-APP UPDATE NOTIFICATION SYSTEM (v1.87.0)
// ============================================================================
(function() {
  const CURRENT_BUILD_VERSION = (pkg && pkg.version) ? pkg.version : "1.87.0";
  ...
  function showUpdateBanner(updateData) {
    ...
    if (localStorage.getItem('wos_dismissed_update') === updateData.version) return;
    if (localStorage.getItem('wos_app_version') === updateData.version) return;
    ...
    document.getElementById('btn-dismiss-update-app').addEventListener('click', () => {
      localStorage.setItem('wos_dismissed_update', updateData.version);
...
window.showAppInstallSelector = function() {
  ...
  // Android Click -> Trigger 1-Click PWA Install or Android Guide
  ...
  // Apple iOS Click -> Open 4-Step iOS Guide Modal
...
// ============================================================================
// ANDROID CHROME NATIVE PWA INSTALL GUIDE SYSTEM (v1.88.3)
// ============================================================================
window.showAndroidInstallGuide = function() {
  if (document.getElementById('android-pwa-install-modal')) return;

  const modal = document.createElement('div');
  modal.id = 'android-pwa-install-modal';
  modal.style.cssText = `
    position: fixed;
    top: 0; left: 0; right: 0; bottom: 0;
    background: rgba(9, 13, 22, 0.88);
  ...
  modal.innerHTML = `
    <div style="background: linear-gradient(135deg, rgba(15, 23, 42, 0.98), rgba(30, 41, 59, 0.98)); border: 1px solid rgba(34, 197, 94, 0.6); box-shadow: 0 25px 60px rgba(0,0,0,0.9), 0 0 35px rgba(34, 197, 94, 0.25); border-radius: 24px; padding: 26px; max-width: 460px; width: 100%; color: #f1f5f9; position: relative;">
      <button id="close-android-install-modal" style="position: absolute; top: 16px; right: 16px; background: rgba(255,255,255,0.1); border: none; color: #94a3b8; width: 32px; height: 32px; border-radius: 50%; cursor: pointer; font-size: 1.1rem; font-weight: bold;">✕</button>
      
      <div style="text-align: center; margin-bottom: 20px;">
        <div style="font-size: 3rem; margin-bottom: 8px;">🤖</div>
        <h2 style="color: #4ade80; font-size: 1.4rem; margin-bottom: 6px; font-weight: 800;">Install App on Android</h2>
        <p style="color: #94a3b8; font-size: 0.88rem; line-height: 1.4;">Add WOS Dashboard directly to your Android home screen!</p>
      </div>

      <button id="btn-trigger-android-native" style="width: 100%; background: linear-gradient(135deg, #22c55e, #10b981); border: none; color: #0f172a; font-weight: 800; font-size: 1.02rem; padding: 14px; border-radius: 16px; cursor: pointer; box-shadow: 0 4px 15px rgba(34, 197, 94, 0.4); margin-bottom: 18px; display:flex; align-items:center; justify-content:center; gap:8px;">
        ⚡ Tap Here to Try 1-Click Install Prompt
      </button>
      ...
      <div style="background: rgba(0,0,0,0.3); border-radius: 16px; padding: 16px; margin-bottom: 18px; border: 1px solid rgba(255,255,255,0.06); display:flex; flex-direction:column; gap:12px;">
        <div style="font-size: 0.82rem; color: #4ade80; font-weight: 700; text-transform: uppercase; letter-spacing: 0.5px; margin-bottom: 2px;">Android Chrome 3-Step Guide:</div>
        <div style="display: flex; align-items: flex-start; gap: 10px;">
          <div style="background: rgba(34, 197, 94, 0.15); border: 1px solid #4ade80; color: #4ade80; width: 24px; height: 24px; border-radius: 50%; display: flex; align-items: center; justify-content: center; font-weight: 800; font-size: 0.8rem; flex-shrink: 0;">1</div>
          <div style="font-size: 0.86rem; color: #e2e8f0;">Tap the <strong style="color: #4ade80;">Chrome Menu (⋮)</strong> in top right.</div>
        </div>
        <div style="display: flex; align-items: flex-start; gap: 10px;">
          <div style="background: rgba(34, 197, 94, 0.15); border: 1px solid #4ade80; color: #4ade80; width: 24px; height: 24px; border-radius: 50%; display: flex; align-items: center; justify-content: center; font-weight: 800; font-size: 0.8rem; flex-shrink: 0;">2</div>
          <div style="font-size: 0.86rem; color: #e2e8f0;">Tap <strong style="color: #4ade80;">"Install App" 📲</strong> or "Add to Home screen".</div>
        </div>
        <div style="display: flex; align-items: flex-start; gap: 10px;">
          <div style="background: rgba(34, 197, 94, 0.15); border: 1px solid #4ade80; color: #4ade80; width: 24px; height: 24px; border-radius: 50%; display: flex; align-items: center; justify-content: center; font-weight: 800; font-size: 0.8rem; flex-shrink: 0;">3</div>
          <div style="font-size: 0.86rem; color: #e2e8f0;">Tap <strong style="color: #4ade80;">"Install"</strong> to confirm!</div>
        </div>
      </div>

      <button id="btn-close-android-guide" style="width: 100%; background: rgba(255,255,255,0.08); border: 1px solid rgba(255,255,255,0.15); color: #cbd5e1; font-weight: 600; font-size: 0.9rem; padding: 12px; border-radius: 14px; cursor: pointer;">
        Close Guide
      </button>
    </div>
  `;
  ...
  const closeFn = () => modal.remove();
  document.getElementById('close-android-install-modal').addEventListener('click', closeFn);
  document.getElementById('btn-close-android-guide').addEventListener('click', closeFn);

  document.getElementById('btn-trigger-android-native').addEventListener('click', () => {
    if (window.deferredPwaPrompt) {
      window.deferredPwaPrompt.prompt();
      modal.remove();
    } else {
      if (window.showToast) window.showToast("Tap Chrome Menu (⋮) ➔ Install App or Add to Home Screen!", "info");
    }
  });
};

// ============================================================================
// IPHONE SAFARI NATIVE PWA INSTALL GUIDE SYSTEM (v1.88.3)
// ============================================================================
window.showIosInstallGuide = function() {
  if (document.getElementById('ios-pwa-install-modal')) return;

  const modal = document.createElement('div');
  modal.id = 'ios-pwa-install-modal';
  modal.style.cssText = `
    position: fixed;
    top: 0; left: 0; right: 0; bottom: 0;
    background: rgba(9, 13, 22, 0.88);
      ...
      <div style="background: rgba(0,0,0,0.3); border-radius: 16px; padding: 16px; margin-bottom: 18px; border: 1px solid rgba(255,255,255,0.06); display:flex; flex-direction:column; gap:12px;">
        ...
        <div style="background: rgba(255, 215, 0, 0.08); border: 1px solid rgba(255, 215, 0, 0.3); border-radius: 14px; padding: 14px;">
          <div style="font-weight: 800; font-size: 0.92rem; color: #ffd700; margin-bottom: 6px; display:flex; align-items:center; gap:6px;">
            <span>🧭</span> If using Safari (iPhone & iPad):
          </div>
          <div style="font-size: 0.85rem; color: #cbd5e1; line-height: 1.5;">
            1. Tap Safari's <strong style="color:#ffd700; display:inline-flex; align-items:center; gap:4px; vertical-align:middle;">Share <span style="display:inline-flex; align-items:center; justify-content:center; background:rgba(255,215,0,0.18); border:1px solid rgba(255,215,0,0.5); border-radius:6px; padding:2px 5px; vertical-align:middle;"><svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="#ffd700" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><path d="M4 12v8a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2v-8"></path><polyline points="16 6 12 2 8 6"></polyline><line x1="12" y1="2" x2="12" y2="15"></line></svg></span></strong> icon (or tap <strong style="color:#ffd700;">"..."</strong> if Share isn't at the top).<br>
        ...
        <div style="background: rgba(34, 197, 94, 0.08); border: 1px solid rgba(34, 197, 94, 0.3); border-radius: 14px; padding: 14px;">
          <div style="font-weight: 800; font-size: 0.92rem; color: #4ade80; margin-bottom: 6px; display:flex; align-items:center; gap:6px;">
            <span>🌐</span> If using Chrome on iOS:
          </div>
          <div style="font-size: 0.85rem; color: #cbd5e1; line-height: 1.5;">
            1. Tap Chrome's <strong style="color:#4ade80; display:inline-flex; align-items:center; gap:4px; vertical-align:middle;">Share <span style="display:inline-flex; align-items:center; justify-content:center; background:rgba(34,197,94,0.18); border:1px solid rgba(34,197,94,0.5); border-radius:6px; padding:2px 5px; vertical-align:middle;"><svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="#4ade80" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><path d="M4 12v8a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2v-8"></path><polyline points="16 6 12 2 8 6"></polyline><line x1="12" y1="2" x2="12" y2="15"></line></svg></span></strong> icon in top address bar (next to URL).<br>
            2. Tap <strong style="color:#4ade80;">"View More"</strong> in the popup options.<br>
...
(function() {
  ...
  window.showAutoInstallBanner = function(ignoreDismissed = false) {
    ...
    if (document.getElementById('wos-version-update-banner')) {
      setTimeout(() => window.showAutoInstallBanner(ignoreDismissed), 5000);
      return;
    }
    ...
    banner.style.cssText = `
      ...
      max-width: 440px;
      background: linear-gradient(135deg, rgba(15, 23, 42, 0.96), rgba(30, 41, 59, 0.98));
      border: 1px solid ${isApple ? 'rgba(255, 215, 0, 0.7)' : 'rgba(34, 197, 94, 0.7)'};
      box-shadow: 0 15px 40px rgba(0,0,0,0.8), 0 0 25px ${isApple ? 'rgba(255, 215, 0, 0.25)' : 'rgba(34, 197, 94, 0.25)'};
      border-radius: 20px;
      padding: 14px 18px;
      z-index: 999999;
      color: #f1f5f9;
    ...
    banner.innerHTML = `
      <div style="display:flex; align-items:center; gap: 12px;">
        <div style="width: 42px; height: 42px; background: ${isApple ? 'linear-gradient(135deg, #ffd700, #ff8c00)' : 'linear-gradient(135deg, #22c55e, #10b981)'}; border-radius: 12px; display: flex; align-items: center; justify-content: center; font-size: 1.5rem; box-shadow: 0 4px 12px ${isApple ? 'rgba(255,215,0,0.4)' : 'rgba(34,197,94,0.4)'}; flex-shrink:0;">${isApple ? '🍎' : '🤖'}</div>
        <div>
          <div style="font-weight: 800; font-size: 0.95rem; color: ${isApple ? '#ffd700' : '#4ade80'};">${isApple ? 'Install wosBDC on Apple' : 'Install wosBDC App'}</div>
          <div style="font-size: 0.78rem; color: #94a3b8;">${isApple ? 'Tap Share 📤 ➔ Add to Home Screen' : 'Add to Home Screen for 1-Tap Access'}</div>
        </div>
      </div>
      <div style="display:flex; align-items:center; gap: 8px;">
        <button id="btn-pwa-install-now" style="background: ${isApple ? 'linear-gradient(135deg, #ffd700, #ff8c00)' : 'linear-gradient(135deg, #22c55e, #10b981)'}; border: none; color: #0f172a; font-weight: 800; font-size: 0.85rem; padding: 9px 15px; border-radius: 12px; cursor: pointer; box-shadow: 0 3px 10px ${isApple ? 'rgba(255,215,0,0.3)' : 'rgba(34,197,94,0.3)'}; white-space:nowrap;">
          ${isApple ? '📲 How to Add' : '⚡ Install App'}
        </button>
  ...
  window.addEventListener('load', () => {
    setTimeout(() => window.showAutoInstallBanner(), 1500);
  });
})();
